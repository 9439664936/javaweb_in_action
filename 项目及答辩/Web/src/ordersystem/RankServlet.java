		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < countList.size(); i++) {
					if (countList.get(i).getName().equals(rs.getString("food_name"))) {
						int tmp = countList.get(i).getNum() + rs.getInt("food_quantity");
				...
				if (!flag) {
					...
					countBean.setName(rs.getString("food_name"));
					countBean.setNum(rs.getInt("food_quantity"));
